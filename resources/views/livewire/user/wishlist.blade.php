                            ...
                            <div class="flex flex-col items-center justify-between gap-y-2 h-full">
                                <div class="relative">
                                    <img class="rounded w-64 h-64"
                                        src="{{ asset('product_thumbnail/' . $item->model->thumbnail_image) }}" alt="">
                                    ...
                                    <div wire:click.prevent="moveProductFromWishlistToCart('{{ $item->rowId }}')"
                                ...
                                <div class="gap-y-1 flex flex-col items-center">
                                    <h2 class="text-2xl font-light"><a
                                            href="{{ route('detail', [$item->model->slug]) }}">{{ $item->model->name }}</a>
                                    </h2>
                                    @if ($item->model->off_price)
                                        <p class="font-medium line-through text-gray-500">
                                            {{ number_format($item->model->regular_price) }} تومان
                                        </p>
                                        <p class="font-bold text-lg text-red-600">
                                            {{ number_format($item->model->off_price) }} تومان
                                        </p>
                                    @else
                                        <p class="text-lg font-bold">{{ number_format($item->model->regular_price) }}
                                            تومان
                                        </p>
                                    @endif
                                </div>
                                <button wire:click.prevent="removeFromWishlist('{{ $item->rowId }}')"
