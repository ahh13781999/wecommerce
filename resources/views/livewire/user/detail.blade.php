<div x-data="{ chosenImage: '', imageModal: false }">
    <div class="w-full h-full container mx-auto py-4 lg:py-10 px-4 lg:px-12">
        <div class="flex flex-row justify-start items-center gap-x-1 lg:gap-x-2 text-gray-800">
            ...
            <p class="font-semibold text-base lg:text-lg">{{ $product->subcategory->subcategory_name }}</p>
            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24"
                stroke="currentColor" stroke-width="2">
                <path stroke-linecap="round" stroke-linejoin="round" d="M15 19l-7-7 7-7" />
            </svg>
            <p class="font-semibold text-base lg:text-lg">{{ $product->name }}</p>
        </div>
                    ...
                    <div class="grid grid-cols-6 gap-y-10">
                        <div class="col-span-6 lg:col-span-3 order-2 lg:order-1">
                            <div class="flex flex-col gap-y-10 justify-between">
                                <p class="border-b-2 py-6 text-2xl font-semibold">{{ $product->name }}</p>
                                <ul
                                    class="flex flex-col justify-between gap-y-2 py-4 list-inside list-disc text-xl font-normal text-gray-600">
                                    @foreach ($product->attributeValues->take(5) as $item)
                                        <li><span class="text-black font-semibold">{{ $item->ProductAttribute->name }}
                                                : </span>{{ $item->value }}</li>
                                    @endforeach
                                </ul>
                                @if ($product->off_price)
                                    <p class="text-xl font-bold">
                                        ...
                                        <span
                                            class="text-2xl font-black line-through">{{ number_format($product->regular_price) }}</span>
                                        <span>تومان</span>
                                    </p>
                                ...
                                @else
                                    <p class="text-3xl font-black">{{ number_format($product->regular_price) }}
                                        <span>تومان</span>
                                    </p>
                                @endif
                                ...
                                @if ($product->quantity <= 10)
                                    <div class="text-red-800 text-lg">
                                        <p>فقط {{ $product->quantity }} عدد در انبار مانده !</p>
                                    </div>
                                @endif
                                ...
                                <div
                                    ...
                                    @else
                                        <a wire:click.prevent="addToWishlist({{ $product->id }},'{{ $product->name }}',{{ $product->off_price ?? $product->regular_price }})"
                                            href="#"
                                ...
                                @foreach ($product->images->take(4) as $image)
                                    <li
                                        ...
                                        <img x-on:click.prevent="chosenImage = $el.src; imageModal = true"
                                            class="h-28 w-28"
                                            src="{{ asset('product_images/' . $product->name . '/' . $image->image_path) }}"
                                            alt="{{ $product->name }}">
                                    </li>
                                @endforeach
                            </ul>
                    ...
                    <div x-data={tab:1} class="flex flex-col gap-y-1">
                        ...
                        <div class="p-6 border-2">
                                    ...
                                    @foreach ($product->attributeValues as $item)
                                        <tr class="border-b-2 flex flex-row items-center justify-between w-full py-2">
                                            <th class="border-l-2 text-center basis-1/2">
                                                <p class="text-lg font-semibold">{{ $item->ProductAttribute->name }}
                                                </p>
                                            </th>
                            ...
                            <div x-show="tab == 3" class="flex flex-col justify-between gap-y-16">
                                <h2 class="text-xl font-bold mt-6 text-orange-900">{{ $product->reviews->count() }}
                                    نظر</h2>
