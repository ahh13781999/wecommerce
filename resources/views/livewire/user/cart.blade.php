        ...
        <div class="flex flex-col justify-between w-full gap-y-16 my-12">
            <ul class="flex flex-col justify-between">
                ...
                @if (session()->has('message'))
                    <p class="bg-green-300 mb-6 py-2 px-4 rounded font-bold">{{ session()->get('message') }}</p>
                @endif
                        ...
                        <li
                            ...
                            <div class="basis-1/6 flex-1">
                                <img class="md:w-16 md:h-16 w-32 h-32 rounded shadow-lg"
                                    src="{{ asset('product_thumbnail/'.$product->model->thumbnail_image) }}" alt="">
                            </div>
                            <div class="basis-1/6 flex-1">
                                <a class="text-lg font-semibold"
                                    href="{{ route('detail', ['slug' => $product->model->slug]) }}">{{ $product->name }}</a>
                            </div>
                            <div class="basis-1/6 flex-1">
                                <p class="text-lg font-bold text-gray-700">{{ number_format($product->price) }} تومان</p>
                            </div>
                            <div class="flex flex-col items-center gap-y-2 basis-1/6 flex-1">
                                <div class="flex flex-row items-center py-2 px-2 border-2 rounded gap-x-2">
                                    <input value="{{ $product->qty }}"
                                        ...
                                        type="text">
                                    <a wire:click.prevent="decreaseQuantity('{{ $product->rowId }}')"
                                    ...
                                    <a wire:click.prevent="increaseQuantity('{{ $product->rowId }}')"
                                ...
                                <a wire:click.prevent="saveForLater('{{ $product->rowId }}')" href="#" class="font-bold text-center text-red-500 hover:text-black">ذخیره برای بعدا</a>
                            </div>
                            <div class="basis-1/6 flex-1">
                                <p class="text-lg font-bold text-gray-700">{{ number_format($product->subtotal) }} تومان</p>
                            </div>
                            <div class="hidden md:block basis-1/6 flex-1">
                                <a wire:click.prevent="destory('{{ $product->rowId }}')" href="">
                            ...
                            <div class="block md:hidden">
                                <button wire:click.prevent="destory('{{ $product->rowId }}')"
                    ...
                    <div class="col-span-6 md:col-span-2">
                        <div class="flex flex-col justify-between gap-y-4 w-full">
                            ...
                            <div class="flex flex-row justify-between w-full">
                                ...
                                <p class="text-xl font-bold">{{ number_format(Cart::subtotal()) }} تومان</p>
                            </div>
                            @if (session()->has('coupon'))
                            <div class="flex flex-row justify-between w-full">
                                <p class="text-gray-600 text-lg font-medium flex flex-row items-center gap-x-1">تخفیف(<span class="font-bold text-gray-500">{{ session()->get('coupon')['code'] }}</span>) 
                                ...
                                <p class="text-xl font-bold">{{ number_format($discount) }} تومان</p>
                            </div> 
                            <div class="flex flex-row justify-between w-full">
                                <p class="text-gray-600 text-lg font-medium">هزینه ارسال</p>
                                <p class="text-xl font-bold">{{ number_format(Cart::tax()) }} تومان</p>
                            </div>
                            <hr class="w-full">
                            <div class="flex flex-row justify-between w-full">
                                <p class="text-gray-600 text-lg font-medium">هزینه کل</p>
                                <p class="text-xl font-bold">{{ number_format($totalAfterDiscount) }} تومان</p>
                            </div>
                            @else
                            <div class="flex flex-row justify-between w-full">
                                <p class="text-gray-600 text-lg font-medium">هزینه ارسال</p>
                            ...
                            <div class="flex flex-row justify-between w-full">
                                <p class="text-gray-600 text-lg font-medium">هزینه کل</p>
                                <p class="text-xl font-bold">{{ number_format(Cart::total()) }} تومان</p>
                            </div>
                            @endif
                        </div>
                    </div>
                ...
                @if (Cart::instance('saveForLater')->count() > 0)
                    ...
                    @foreach (Cart::instance('saveForLater')->content() as $product)
                        <li
                            ...
                            <div class="">
                                <p class="text-lg font-bold text-gray-700">{{ number_format($product->model->off_price) ?? number_format($product->model->regular_price) }} تومان</p>
                            </div>
                            <div class="flex flex-col items-center">
                                <a wire:click.prevent="moveToCart('{{ $product->rowId }}')" href="#" class="font-bold text-center text-red-500 hover:text-black">اضافه به لیست خرید</a>
                            </div>
                            <div class="">
                                <p class="text-lg font-bold text-red-700">{{ $product->qty }} عدد</p>
                            </div>
