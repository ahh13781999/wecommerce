<div>
    <div x-data class="w-full h-full container mx-auto py-4 lg:py-10 px-4 lg:px-12">
        <div class="flex flex-row justify-start items-center gap-x-1 lg:gap-x-2 text-gray-800">
            ...
            <p class="font-semibold text-base lg:text-lg">{{ $sub_category->category->category_name }}</p>
            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24"
                stroke="currentColor" stroke-width="2">
                <path stroke-linecap="round" stroke-linejoin="round" d="M15 19l-7-7 7-7" />
            </svg>
            <p class="font-semibold text-base lg:text-lg">{{ $sub_category->subcategory_name }}</p>
        </div>
                                ...
                                <div x-ref="subcategory" class="hidden">
                                    @foreach ($category->subcategories as $subcategory)
                                        <li class="text-base font-bold text-stone-400 hover:bg-gray-200 p-1 my-2 <?php if(Request::segment(2) == $subcategory->subcategory_slug){ echo "bg-gray-300 text-black"; } ?>">
                                            <a href="{{ route('sub-category', ['slug' => $subcategory->subcategory_slug]) }}">-
                    ...
                    <div class="flex flex-col items-center px-4 w-full">
                        ...
                        <div class="bg-gray-200 py-2 px-4 w-full mt-6">
                            <div class="flex flex-row items-center justify-between">
                                <h2 class="text-xl font-semibold">{{ $sub_category->subcategory_name }}</h2>
                        ...
                        @foreach ($products as $product)
                            <div
                                ...
                                @if ($wishListItems->contains($product->id))
                                    <svg wire:click.prevent="removeFromWishlist({{ $product->id }})"
                                ...
                                @else
                                    <svg wire:click.prevent="addToWishlist({{ $product->id }},'{{ $product->name }}',{{ $product->off_price ?? $product->regular_price }})"
                                        xmlns="http://www.w3.org/2000/svg"
